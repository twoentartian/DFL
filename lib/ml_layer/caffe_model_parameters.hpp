...
namespace Ml
{
	...
	{
	public:
		caffe_parameter_blob() : _empty(true) {};
	...
	public:
		...
		void fromLayer(const caffe::Layer<DType>& layer)
		{
			...
			_blob_p.reset(new caffe_parameter_blob<DType>());
			if (blobs.size() == 0)
			{
				return;
			}
	...
	private:
		...
		std::string _name;
		std::string _type;
		boost::shared_ptr<caffe_parameter_blob<DType>> _blob_p;
	};
