...
//        for (int i=0;i<models.size();i++)
//        {
//            drop_list.push_back(distribution(rng));
//        }
//
//        int number_valid_peers=0;
//
//
//        for (int i = 0; i < models.size(); ++i)
//        {
...
////        average_reputation /= models.size();
//
//        average_accuracy /= number_valid_peers;
//        average_reputation /= number_valid_peers;
//
//
//        int counter_normal = 0, counter_filtered = 0;
//        for (int i = 0; i < models.size(); ++i)
...
//                if (models[i].type == Ml::model_compress_type::normal) {
//                    counter_normal++;
//                    parameter_buffers.add(models[i].model_parameter * (1 + models[i].accuracy - average_accuracy) *
//                                          (1 + reputation_copy[models[i].generator_address] - average_reputation));
//                } else if (models[i].type == Ml::model_compress_type::compressed_by_diff) {
//                    counter_filtered++;
//                    parameter_buffers_filtered.add(
//                            models[i].model_parameter * (1 + models[i].accuracy - average_accuracy) *
//                            (1 + reputation_copy[models[i].generator_address] - average_reputation));
//                } else {
//                    LOG(WARNING) << "[REPUTATION SDK] unknown model type";
//                }
...
//    }
    ...
    void update_model(Ml::caffe_parameter_net<DType>& current_model, double self_accuracy, const std::vector<updated_model<DType>>& models, std::unordered_map<std::string, double>& reputation) override
    {
        ...
        for (int i = 0; i < models.size(); ++i)
        {
            ...
            {
                counter_normal++;
                float temp_weight= (1 + models[i].accuracy - average_accuracy) * (1 + reputation_copy[models[i].generator_address] - average_reputation);
        ...
        {
            weights[i]=weights[i]/total_weight*weights.size();
        }
